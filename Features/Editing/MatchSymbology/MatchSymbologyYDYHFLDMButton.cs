...
namespace XIAOFUTools.Features.Editing.MatchSymbology
{
    internal class MatchSymbologyYDYHFLDMButton : Button
    {
        // 相对路径字段，指向符号库位置
        private string _relativePath = @"Assets\Data\符号库\";
        ...
        protected override async void OnClick()
        {
            ...
            // 组合完整路径
            string fullStylePath = Path.Combine(installPath, _relativePath, "用地用海.stylx");
            ...
            await QueuedTask.Run(() =>
            {
                ...
                // 检查字段是否存在于选中的图层中
                return fields.Any(field => field.Name.Equals("YDYHFLDM", StringComparison.OrdinalIgnoreCase));
            });
            if (!fieldExists)
            {
                // 使用 ArcGIS Pro 的消息框
                MessageBox.Show("字段 'YDYHFLDM' 不存在于选中的图层中。");
                return;
            }
            ...
            var parameters = Geoprocessing.MakeValueArray(
                layerUri,      // 图层路径
                "YDYHFLDM",        // 字段名，默认为 YDYHFLDM，且不区分大小写
