...
{
    internal class MatchSymbologyFXDLMCButton : Button
    {
        // 相对路径字段，指向符号库位置
        ...
        protected override async void OnClick()
        {
            ...
            await QueuedTask.Run(() =>
            {
                ...
                // 检查字段是否存在于选中的图层中
                return fields.Any(field => field.Name.Equals("FXDLMC", StringComparison.OrdinalIgnoreCase));
            });
            if (!fieldExists)
            {
                // 使用 ArcGIS Pro 的消息框
                MessageBox.Show("字段 'FXDLMC' 不存在于选中的图层中。");
                return;
            }
