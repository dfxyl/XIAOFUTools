...
namespace XIAOFUTools.Features.DataManagement.MapSheetsLarge
{
    internal class GenerateLargeMapSheetsDockPaneViewModel : PropertyChangedBase
    {
        ...
        private async Task RunAsync()
        {
            ...
            try
            {
                await QueuedTask.Run(async () =>
                {
                    ...
                    // 输出FC
                    using var outputFeatureClass = await CreateOutputFeatureClassAsync(OutputFeatureClassPath, targetSR);
        ...
        private sealed class OutputFeatureClassLease : IDisposable
        {
            private readonly IDisposable _workspace;

            public OutputFeatureClassLease(FeatureClass featureClass, IDisposable workspace)
            {
