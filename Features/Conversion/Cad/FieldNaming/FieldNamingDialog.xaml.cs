...
namespace XIAOFUTools.Features.Conversion.Cad.FieldNaming
{
    public partial class FieldNamingDialog : Window
    {
        ...
        private void FieldsListBox_Drop(object sender, DragEventArgs e)
        {
            if (VM == null) return;
            if (sender is not ListBox lb) return;
            var data = e.Data.GetData(typeof(CadFieldOption)) as CadFieldOption;
            if (data == null) return;
            ...
            if (targetItem != null)
            {
                var targetData = lb.ItemContainerGenerator.ItemFromContainer(targetItem) as CadFieldOption;
    ...
    {
        public ObservableCollection<CadFieldOption> Fields { get; set; } = new();
        ...
        internal static FieldNamingDialogViewModel FromMainVM(ICadFieldNamingTarget vm)
        {
            ...
            {
                dlgVm.Fields.Add(new CadFieldOption { Name = f.Name, Alias = f.Alias, IsSelected = f.IsSelected });
            }
            return dlgVm;
        }

        internal void ApplyToMainVM(ICadFieldNamingTarget vm)
        {
            ...
            {
                vm.NamingFields.Add(new CadFieldOption { Name = f.Name, Alias = f.Alias, IsSelected = f.IsSelected });
            }
        }
