    ...
    internal class ProtocolLineExtractViewModel : PropertyChangedBase
    {
        ...
        private void BrowseOutput()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(init)) dlg.InitialLocation = init;
        ...
        private async Task ProcessCoreAsync(CancellationToken token)
        {
            ...
            {
                // 1. 准备临时目录
                tempWS = Path.Combine(Path.GetTempPath(), "ProtocolLine_" + Guid.NewGuid().ToString("N").Substring(0, 8));
                Directory.CreateDirectory(tempWS);
            ...
            finally
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(tempWS) && Directory.Exists(tempWS))
                    {
                        try { Directory.Delete(tempWS, true); } catch { /* 忽略 */ }
                    }
                }
                catch { }
            }
