    ...
    internal class BatchAddDataViewModel : PropertyChangedBase
    {
        ...
        private void BrowseFolder()
        {
            try
            {
                ...
                {
                    Title = "选择要扫描的文件夹",
                    InitialLocation = string.IsNullOrWhiteSpace(SearchPath) ? null : GetNormalizedSearchPath(),
                    MultiSelect = false,
                    Filter = ItemFilters.Folders
                };

                if (dialog.ShowDialog() == true && dialog.Items?.Count > 0)
                {
                    SearchPath = dialog.Items[0].Path;
                }
            }
            catch (Exception ex)
            {
        ...
        private bool ValidateSearchPath()
        {
            try
            {
                ...
                {
                    var normalizedPath = SearchPath.EndsWith(@"\") ? SearchPath : SearchPath + @"\";
