...
### MDB批量转GDB 工具优化 (2026-03-19)
- 将 `MDB批量转GDB` 转换链路切换为 ArcGIS Pro Python 引擎，替换原有 GDAL 直写方案，减少运行时依赖与打包问题。
- 优化 MDB 结构识别逻辑，改为优先从 `GDB_Items` 读取要素集与图层目录关系，修复中文图层位于要素集内时被误判到根目录的问题。
- 优化元数据与编码兼容处理，补充中文名称/字段别名解析兜底，并在 XML 不完整时仍尽量保留要素集结构。
- 当前对曲线几何仍采用线性化写入，以保证转换稳定性；已验证样例库中的多要素集结构可正确保留。
- 修改的文件：
  - `Tools/DataProcessing/MdbBatchToGdb/*` - 重构 MDB 转 GDB 执行链路、环境探测、批量转换与界面调用逻辑。
  - `scripts/mdb_to_gdb_arcgispro.py` - 新增 ArcGIS Pro Python 转换脚本，补充要素集识别、编码兜底与写入流程。
  - `scripts/tests/test_mdb_to_gdb_arcgispro.py` - 补充元数据解析与要素集映射测试。
  - `XIAOFUTools.Tests/DataProcessing/*` - 补充 ArcGIS Pro Python 环境与相关转换测试。
